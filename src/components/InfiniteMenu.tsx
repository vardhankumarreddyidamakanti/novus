...
class InfiniteGridMenuClass {
  ...
  discLocations: any;
  discGeo: DiscGeometry | undefined;
  discBuffers: any;
  discVAO: WebGLVertexArrayObject | null = null;
  icoGeo: IcosahedronGeometry | undefined;
  ...
  atlasSize: number = 0;
  control: ArcballControl | undefined;
  drawBufferSize: vec2 | undefined;

  constructor(canvas: HTMLCanvasElement, items: any[], onActiveItemChange: (index: number) => void, onMovementChange: (isMoving: boolean) => void, onInit: ((sk: InfiniteGridMenuClass) => void) | null = null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (ctx) ctx.drawImage(img, x, y, cellSize, cellSize);
      });
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenuClass(
        canvas,
